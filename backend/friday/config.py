...
from friday.internal.db import Base, get_db
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (Friday + Ollama)
...
RAG_SEARCH_ALL_BY_DEFAULT = PersistentConfig(
    "RAG_SEARCH_ALL_BY_DEFAULT",
    "rag.search_all_by_default",
    os.environ.get("RAG_SEARCH_ALL_BY_DEFAULT", "True").lower() == "true",
)

RAG_ENABLE_WEB_FALLBACK = PersistentConfig(
    "RAG_ENABLE_WEB_FALLBACK",
    "rag.enable_web_fallback",
    os.environ.get("RAG_ENABLE_WEB_FALLBACK", "True").lower() == "true",
)

RAG_WEB_FALLBACK_THRESHOLD = PersistentConfig(
    "RAG_WEB_FALLBACK_THRESHOLD",
    "rag.web_fallback_threshold",
    float(os.environ.get("RAG_WEB_FALLBACK_THRESHOLD", "0.5")),
)
