...
from friday.models.folders import Folders
from friday.models.users import Users
from friday.socket.main import (
    get_event_call,
    get_event_emitter,
    get_active_status_by_user_id,
)
from friday.routers.tasks import (
    generate_queries,
    generate_title,
    generate_follow_ups,
...
from friday.routers.retrieval import (
    process_web_search,
    SearchForm,
)
from friday.routers.images import (
    image_generations,
    CreateImageForm,
    image_edits,
    EditImageForm,
...
from friday.models.users import UserModel
# Functions removed in Friday rebrand
# from open_webui.models.functions import Functions
from friday.models.models import Models

from friday.retrieval.utils import get_sources_from_items


from friday.utils.chat import generate_chat_completion
from friday.utils.task import (
    get_task_model_id,
    rag_template,
    tools_function_calling_generation_template,
)
from friday.utils.misc import (
    deep_update,
    extract_urls,
    get_message_list,
...
from friday.utils.tools import get_tools, get_updated_tool_function
from friday.utils.plugin import load_function_module_by_id
from friday.utils.filter import (
    get_sorted_filter_ids,
    process_filter_functions,
)
# Code interpreter removed in Friday rebrand
# from open_webui.utils.code_interpreter import execute_code_jupyter
from friday.utils.payload import apply_system_prompt_to_body
from friday.utils.mcp.client import MCPClient
...
# Code interpreter removed in Friday rebrand - define missing constants as None
DEFAULT_CODE_INTERPRETER_PROMPT = None
CODE_INTERPRETER_BLOCKED_MODULES = None

# Code interpreter removed in Friday rebrand - stub function
async def execute_code_jupyter(*args, **kwargs):
    """Stub function - code interpreter disabled in Friday"""
    return {"stdout": "Code interpreter has been disabled in Friday"}
...
from friday.constants import TASKS
...
async def chat_unified_knowledge_search_handler(
    request: Request, form_data: dict, extra_params: dict, user
):
    """
    Search all knowledge bases if no files selected (unified search).

    This handler implements the priority-based search strategy:
    1. Skip if files already selected (respects explicit selection)
    2. Search ALL user's knowledge bases by default
    3. Check relevance threshold
    4. Allow web search fallback if results below threshold
    """
    try:
        # Check if unified search is enabled
        if not request.app.state.config.RAG_SEARCH_ALL_BY_DEFAULT:
            return form_data

        # Skip if files already selected (explicit file selection takes priority)
        if form_data.get("files"):
            log.info("Files already selected, skipping unified knowledge search")
            return form_data

        event_emitter = extra_params["__event_emitter__"]

        # Emit status: searching knowledge base
        await event_emitter({
            "type": "status",
            "data": {
                "action": "unified_knowledge_search",
                "description": "Searching your knowledge base",
                "done": False,
            },
        })
        ...
        if not user_message or user_message.strip() == "":
            log.warning("Empty user message, skipping unified search")
            await event_emitter({
                "type": "status",
                "data": {
                    "action": "unified_knowledge_search",
        ...
        from friday.retrieval.unified_search import unified_search

        # Perform unified search across all collections
        results = await unified_search(
            ...
            threshold=request.app.state.config.RAG_WEB_FALLBACK_THRESHOLD,
            hybrid_search=request.app.state.config.ENABLE_RAG_HYBRID_SEARCH,
            reranking_function=request.app.state.RERANKING_FUNCTION if hasattr(request.app.state, 'RERANKING_FUNCTION') else None,
            k_reranker=request.app.state.config.RAG_TOP_K_RERANKER if hasattr(request.app.state.config, 'RAG_TOP_K_RERANKER') else 3,
        ...
        log.exception(f"Error in unified knowledge search: {e}")
        ...
        try:
            await event_emitter({
                ...
                "data": {
                    "action": "unified_knowledge_search",
                    "description": f"Error searching knowledge base: {str(e)}",
                    "done": True,
                },
            })
        except:
        ...
        # Don't fail the request, just skip unified search
        return form_data
