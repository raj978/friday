...
class TestAuths(AbstractPostgresTest):
    ...
    def test_get_session_user(self):
        ...
        assert response.json() == {
            "id": "1",
            "name": "John Doe",
            "email": "john.doe@friday.com",
            "role": "user",
            "profile_image_url": "/user.png",
        }

    def test_update_profile(self):
        from friday.utils.auth import get_password_hash
    ...
    def test_update_password(self):
        ...
        old_auth = self.auths.authenticate_user(
            "john.doe@friday.com", "old_password"
        )
        assert old_auth is None
        new_auth = self.auths.authenticate_user(
            "john.doe@friday.com", "new_password"
        )
    ...
    def test_signin(self):
        ...
        response = self.fast_api_client.post(
            ...
            json={"email": "john.doe@friday.com", "password": "password"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == user.id
        assert data["name"] == "John Doe"
        assert data["email"] == "john.doe@friday.com"
    ...
    def test_add_user(self):
        ...
        assert data["email"] == "john.doe2@friday.com"
